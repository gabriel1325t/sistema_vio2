...
module.exports = class userController {
  ...
  static async updateUser(req, res) {
    // Desestrutura e recupera os dados envados via corpo da requisição
    const { cpf, email, password, name } = req.body;
    ...
    if (userIndex === -1) {
      return res.status(400).json({ error: "Usuáro não encontrado" });
    }
  ...
  static async deleteUser(req, res) {
    //obtém o parametro 'id' da requisição, que é o cpf do user a ser deetado
